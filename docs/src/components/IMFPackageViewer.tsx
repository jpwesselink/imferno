...
const SequenceTimeline = ({sequences,maxDuration,tracks,editRate}: any) => {
  ...
  const buildLabel = (seq: any) => { const p=[seqTypeLabel(seq.type)],l=seqMeta.lang[seq.sequenceNumber]||(seq.language?seq.language.toUpperCase():null),m=seqMeta.mca[seq.sequenceNumber],k=seqMeta.kind[seq.sequenceNumber]; if(l)p.push(l); if(seq.channels&&!m)p.push(seq.channels); if(m)p.push(m); if(k&&k!=="PRM")p.push(contentKindLabel(k)); return p.join(" \u00b7 "); };
