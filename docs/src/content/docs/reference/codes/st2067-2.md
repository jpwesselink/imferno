...
1. **Package-level codes** (`St2067_2_2020`) — AssetMap, PKL, and file-integrity checks emitted by `imf-parser`.
2. **Core Constraints CPL codes** — CPL structure rules shared across all three spec editions (2013 / 2016 / 2020), emitted by the CoreConstraints validators in `st2067-21`. The only difference between editions is the year in the code prefix.
