...
pub fn run() {
    ...
    let js_path = out_dir.join("codes.js");
    let dts_path = out_dir.join("codes.d.ts");

    fs::write(&js_path, js).unwrap_or_else(|e| panic!("write codes.js: {e}"));
    fs::write(&dts_path, dts).unwrap_or_else(|e| panic!("write codes.d.ts: {e}"));
